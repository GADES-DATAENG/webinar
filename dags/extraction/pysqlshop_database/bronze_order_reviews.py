...
LOCAL_CSV_PATH = f"{Variable.get("LOCAL_FILE_PATH")}/olist_order_reviews_dataset.csv"
PROJECT_ID = Variable.get("GCP_PROJECT_ID")
BRONZE_DATASET = Variable.get("BRONZE_BIGQUERY_DATASET")
BIGQUERY_TABLE = f"{PROJECT_ID}.{BRONZE_DATASET}.bronze_order_reviews"
...
def generate_unique_file_name(**kwargs):
    ...
    unique_file_name = f"order_reviews/data_{unique_id}_{timestamp}.csv"
...
with DAG(
    dag_id="bronze_order_reviews",
    start_date=days_ago(1),
    schedule_interval=None,
    catchup=False,
    tags=["bigquery", "pysqlshop", "bronze_layer", "order_reviews"],
) as dag:
    ...
    load_to_bq = PythonOperator(
        ...
        dag=dag,
        outlets=[Dataset("bronze_order_reviews_dataset_ready")] # This indicates that this task writes to the dataset
    )
